...
usersRouter.get("/", JWTAuthMiddleware, async (req, res, next) => {
  try {
    ...
    const result = await Model.find(query.criteria)
      ...
      .limit(
        query.options.limit && query.options.limit < maxLimit
          ? query.options.limit
          : maxLimit
      )
    ...
    res
      .status(200)
      .send({ navigation: query.links("/users", pages), pages, response })
  } catch (error) {
    next(error)
  }
})
...
usersRouter.post("/me/avatar", upload, async (req, res, next) => {
  try {
    let user = req.user
    user.avatar = await Model.findByIdAndUpdate(
      req.user._id,
